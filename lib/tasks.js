// ▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬
// lib/tasks.js — задания (callback + пошаговое создание)
// ▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬
...
const { notifyNewTask } = require('./notifications');
const { notifyUser } = require('./notify');
const { renderTaskCard, buildTaskKeyboard, getTaskWithDetails } = require('./taskCard');
const { sendVideo, deleteMessage } = require('./telegram');

const handleTasksCallback = async (data, ctx) => {
  const { edit, user, chatId, messageId, isModerator, userState, sendMessage } = ctx;
  ...
  if (data.startsWith('task_')) {
    ...
    try {
      ...
      if (!task) {
        await edit('❌ *Задание не найдено*', 'Markdown', {
          inline_keyboard: [[{ text: '◀ СПИСОК', callback_data: 'tasks' }]],
        });
        return true;
      }

      const { text } = renderTaskCard(task, user);
      const keyboard = buildTaskKeyboard(task, user);

      if (task.videoUrl && messageId) {
        await deleteMessage(chatId, messageId);
        await sendVideo(chatId, task.videoUrl, text, keyboard);
      } else {
        await edit(text, 'Markdown', keyboard);
      }
    } catch (e) {
      console.error('task_ view:', e);
  ...
  if (data.startsWith('take_')) {
    ...
    try {
      ...
      await notifyUser(t.creatorId, {
        message: `🎯 Задание "${t.title}" взято игроком ${user.displayName || user.name}`,
  ...
  if (data.startsWith('abandon_')) {
    ...
    try {
      ...
      await notifyUser(t.creatorId, {
        ...
        pushText: `↩️ *Игрок отказался от задания*\n📌 ${t.title}\n\n_Задание снова открыто._`,
        type: 'task',
        icon: '↩️',
        linkType: 'task',
        linkId: t.id,
      ...
      if (approve >= 5) {
        ...
        if (tr.rowCount > 0) {
          ...
          await notifyUser(t.creatorId, {
            ...
            pushText: `✅ *Задание выполнено!*\n📌 ${t.title}\n\n_Не забудь оставить отзыв игроку — кнопка «⭐ Оставить отзыв» в профиле._`,
            type: 'task',
            icon: '✅',
            linkType: 'task',
            linkId: t.id,
      ...
      if (reject >= 5) {
        ...
        if (tr.rowCount > 0) {
          ...
          await notifyUser(t.playerId, {
            ...
            pushText: `❌ *Задание отклонено*\n📌 ${t.title}\n\n_Зрители проголосовали против. Можешь попробовать другое задание._`,
            type: 'task',
            icon: '❌',
            linkType: 'task',
            linkId: t.id,
          ...
          // Уведомление создателю — деньги возвращаются
...
const handleCreateTaskStep = async (state, text, ctx) => {
  ...
  if (state.step === 'reward') {
    ...
    if (reward >= 500) {
      const sent = await notifyNewTask(t.id, t.title, t.reward, user.displayName || user.name);
      console.log(`Push отправлен ${sent} пользователям`);
    }
