// ▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬
// lib/profile.js — профиль + отзывы + настройки + ник + поиск
// ▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬
...
const handleProfileCallback = async (data, ctx) => {
  ...
  if (data === 'profile') {
    ...
    if (user.bio) {
      text += `📝 _${user.bio.slice(0, 100)}${user.bio.length > 100 ? '…' : ''}_\n\n`;
    }
    text += `🎖 Уровень: *${level}*\n`;
    text += `${bar} ${pct}%\n`;
  ...
  if (data === 'edit_profile') {
    ...
    const text =
      ...
      `▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n\n` +
      `📸 *Аватарка:*\n` +
      `Отправь фото в чат бота — оно станет твоей аватаркой.\n\n` +
      `📝 *Bio (описание):*\n` +
      `Отправь команду:\n` +
      `\`/bio Твой текст о себе\`\n` +
      `_До 200 символов_\n\n` +
      `🗑 *Удалить:*\n` +
      `\`/clearbio\` — удалить описание\n` +
      `\`/clearavatar\` — удалить аватарку\n\n` +
      `💡 _Аватарку можно просто перезаписать, отправив новое фото._`;
    await edit(text, 'Markdown', {
      inline_keyboard: [[{ text: '🔙 К профилю', callback_data: 'profile' }]],
    });
    return true;
  ...
  // ═══════════ ПРОСМОТР ПРОФИЛЯ ИГРОКА (из поиска) ═══════════
  if (data.startsWith('view_user_')) {
    if (!user) { await edit('❌', 'Markdown', { inline_keyboard: [] }); return true; }
    const targetId = parseInt(data.replace('view_user_', ''), 10);
    if (!targetId) {
      await edit('❌ *Неверный ID*', 'Markdown', { inline_keyboard: [[{ text: '🔙', callback_data: 'menu' }]] });
      return true;
    }
    ...
    try {
      const r = await query(
        `SELECT u.id, u.name, COALESCE(u."displayName", u.name) AS display,
                ...
                u."loginStreak", u."isModerator", u.avatar, u.bio, u."createdAt",
                u."ratingAvg", u."ratingCount", u."completedTasksCount",
                pres."lastSeen",
      ...
      if (r.rows.length === 0) {
        await edit('❌ *Игрок не найден*', 'Markdown', {
      ...
      const online = u.lastSeen
        ? (Date.now() - new Date(u.lastSeen).getTime()) / 60000 < 15
        : false;
      const roleLabel = u.role === 'player' ? '🎮 Игрок' : u.role === 'viewer' ? '👁 Зритель' : '⚙️ Админ';
      const ratingLine = u.ratingCount > 0
        ? `⭐ Рейтинг: ${Number(u.ratingAvg).toFixed(1)} (${u.ratingCount})`
        : `⭐ Рейтинг: пока нет`;

      let msg = `👤 *@${display}*`;
      if (u.isModerator) msg += ' 👮';
      msg += '\n';
      if (u.bio) msg += `_${u.bio.slice(0, 150)}${u.bio.length > 150 ? '…' : ''}_\n`;
      msg += `▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n\n`;
      msg += `${roleLabel}\n`;
      msg += `🎖 Уровень: *${u.level || 1}* (${u.experience || 0} XP)\n`;
      msg += `⭐ Репутация: *${u.reputation}*\n`;
      msg += `${ratingLine}\n`;
      msg += `✅ Выполнено заданий: *${u.completedTasksCount || 0}*\n`;
      msg += `🎖 Достижений: *${u.achievements}*\n`;
      msg += `🔥 Streak: *${u.loginStreak || 0} дн.*\n`;
      if (online) msg += `🟢 *Сейчас онлайн*\n`;
      msg += `📅 В NERV с: ${new Date(u.createdAt).toLocaleDateString('ru-RU')}`;

      await edit(msg, 'Markdown', {
        inline_keyboard: [
          [{ text: '🔍 Искать ещё', callback_data: 'menu' }],
          [{ text: '🔙 В меню', callback_data: 'menu' }],
        ],
      });
    } catch (e) {
      console.error('view_user:', e);
      await edit('❌ *Ошибка загрузки*', 'Markdown', {
        inline_keyboard: [[{ text: '🔙 В меню', callback_data: 'menu' }]],
      });
    }
    return true;
