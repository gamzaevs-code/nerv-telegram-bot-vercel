...
const notifyLevelUp = async (userId, levelInfo, sendMessageFn) => {
  ...
  try {
    ...
    await notifyUser(userId, {
      message: `🎉 Уровень ${levelInfo.newLevel}! Бонус +${bonus} ₽`,
      pushText: `🎉 *Уровень повышен!*\n\nТы достиг *${levelInfo.newLevel}* уровня!\n💰 Бонус: +${bonus} ₽`,
      type: 'levelup',
      icon: '🎉',
      linkType: 'profile',
    });
...
const notifyAchievements = async (userId, achievements, sendMessageFn) => {
  ...
  try {
    for (const a of achievements) {
      await notifyUser(userId, {
        message: `🎖 Достижение: ${a.name} (+${a.reward} ₽)`,
        pushText: `🎖 *Новое достижение!*\n\n${a.icon || '🏅'} *${a.name}*\n💰 +${a.reward} ₽`,
        type: 'achievement',
        icon: a.icon || '🎖',
        linkType: 'profile',
      });
...
const notifyReferralEarnings = async (earnings, sendMessageFn) => {
  ...
  for (const e of earnings) {
    ...
    try {
      await notifyUser(e.userId, {
        message: `💸 Реферальный бонус (уровень ${e.level}): +${e.bonus} ₽`,
        pushText: `💸 *Реферальный бонус!*\n\nУровень: *${e.level}*\n💰 +${e.bonus} ₽`,
        type: 'system',
        icon: '💸',
        linkType: 'profile',
      });
...
const processReferralEarnings = async (userId, sourceAmount) => {
  try {
    ...
    for (const { level, percent } of levels) {
      ...
      earnings.push({ level, userId: referrerId, bonus });
...
const PLATFORM_COMMISSION_RATE = 0.11;
